 ...
 * RCS:          $Id: PingManager.java,v 1.22 2010/03/07 21:41:56 drc915 Exp $
...
public class PingManager {
	...
	public synchronized static PingManager getInstance(){
	 ...
	 /* (non-Javadoc)
	 * @see edu.northwestern.ono.util.IPingManager#doPing(java.lang.String, edu.northwestern.ono.util.PluginInterface, edu.northwestern.ono.util.Util.PingResponse)
	 */
    public void doPing(final String ipToPing, final PluginInterface pi, 
    		final PingResponse callback) {
    
    	if (stopped || OnoConfiguration.getInstance().getMaxPings() <=0){
    		callback.response(-1);
    		return;
    	}
    	// check cache first
...
//					}
	    	...
	    	while (activePings >= OnoConfiguration.getInstance().getMaxPings() 
	    			&& !stopped){
	    		try {
    	...
    	if (stopped || MainGeneric.isShuttingDown()){
    		activePings--;
    		callback.response(-2);
    		return;
    	}
    	MainGeneric.createThread("Ping-" +ipToPing, new Runnable(){
	...
	/* (non-Javadoc)
	 * @see edu.northwestern.ono.util.IPingManager#getPingCache()
	 */
