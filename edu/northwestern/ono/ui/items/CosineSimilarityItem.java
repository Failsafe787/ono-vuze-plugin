 ...
 * RCS:          $Id: CosineSimilarityItem.java,v 1.8 2010/03/29 16:48:03 drc915 Exp $
...
TableCellRefreshListener {
	...
	public void refresh(TableCell cell) {
		...
		if (OnoPeerManager.getInstance().getOnoPeer(op.getIp())==null){
			OnoPeerManager.getInstance().addPeer(op);
		}
		String sText = OnoPeerManager.getInstance().getOnoPeer(op.getIp()).printCosSim(true);
		if (sText.length()<=2 && op.isCIDRMatch()) sText = "IP Match";
		//		= op.printCosSim(true);

		if (cell.setText(sText) || !cell.isValid()) {

			if (sText.length()<=2) cell.setSortValue(-1.0);
			else cell.setSortValue(op.getMaxCosSim()==null?sText:op.getMaxCosSim().getValue());
