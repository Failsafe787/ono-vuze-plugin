...
/**
 * An IView with a sortable table. Handles composite/menu/table creation and
 * management.
 * <P>
 * Usage of this class is rather haphazard. Sometimes, you can set a variable to
 * change behavior. Other times you call a function. Some functions are meant
 * for extending or implementing, others are meant as normal functions.
 * 
 * @author Olivier (Original PeersView/MyTorrentsView/etc code)
 * @author TuxPaper 2004/Apr/20: Remove need for tableItemToObject 2005/Oct/07:
 *         Virtual Table 2005/Nov/16: Moved TableSorter into TableView
 * 
 * @todo Remove TableSorter.java, SortableTable.java from CVS
 * 
 * @note From TableSorter.java:<br>
 *       <li>2004/Apr/20: Remove need for tableItemToObject (store object in
 *       tableItem.setData)
 ...
 *         has passed. Currently, dataSourcesToRemove is processed every refresh
 *         IF the table is visible, or it is processed when we collect 20 items
 *         to remove.
 */
public class TableView extends AbstractIView implements ParameterListener,
	 ...
	 * of the table this class is handling. Config settings are stored with the
	 * prefix of "Table.<i>TableID</i>"
	 */
	...
	/**
	 * Link DataSource to their row in the table. key = DataSource value =
	 * TableRowCore
	 */
	...
	/**
	 * For updating GUI. Some UI objects get updating every X cycles (user
	 * configurable)
	 */
	...
	/**
	 * Check Column Widths every 10 seconds on Pre 3.0RC1 on OSX if view is
	 * active. Other OSes can capture column width changes automatically
	 */
	private int checkColumnWidthsEvery = (Constants.isOSX && SWT.getVersion() < 3054) ? 10000 / configMan
			.getIntParameter("GUI Refresh")
			: 0;
	...
	/**
	 * We need to remember the order of the columns at the time we added them in
	 * case the user drags the columns around.
	 */
	...
	// XXX Remove after column selection is no longered triggered on column
	// resize (OSX)
	 ...
	 *            Config settings are stored with the prefix of "Table.<i>TableID</i>"
	 * @param _sPropertiesPrefix
	 *            Prefix for retrieving text from the properties file
	 *            (MessageText). Typically <i>TableID</i> + "View"
	 * @param _basicItems
	 *            Column Definitions
	 * @param _sDefaultSortOn
	 *            Column name to sort on if user hasn't chosen one yet
	 * @param _iTableStyle
	 *            SWT style constants used when creating the table
	 */
	public TableView(String _sTableID, String _sPropertiesPrefix,
	...
	private Composite createSashForm(final Composite composite) {
		...
		int iSplitAt = configMan.getIntParameter(
				sPropertiesPrefix + ".SplitAt", 3000);
		// Was stored at whole
		if (iSplitAt < 100)
			iSplitAt *= 100;
		...
		sash.addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(SelectionEvent e) {
				...
				if (tabFolder.getMinimized()) {
					...
					configMan.setParameter(sPropertiesPrefix
							+ ".subViews.minimized", false);
				}

				Rectangle area = form.getClientArea();
				tabFolderData.height = area.height - e.y - e.height
						- iFolderHeightAdj;
				form.layout();
		...
		tabFolder.addMouseListener(new MouseAdapter() {
			public void mouseDown(MouseEvent e) {
				if (tabFolder.getMinimized()) {
					...
					// If the user clicked down on the restore button, and we
					// restore
					// before the CTabFolder does, CTabFolder will minimize us
					// again
					// There's no way that I know of to determine if the mouse
					// is
					// on that button!
		...
		if (pluginViews != null) {
			String[] sNames = (String[]) pluginViews.keySet().toArray(
					new String[0]);
			for (int i = 0; i < sNames.length; i++) {
				...
				if (l != null) {
					try {
						UISWTViewImpl view = new UISWTViewImpl(sTableID,
								sNames[i], l);
						addTabView(view);
					} catch (Exception e) {
						// skip, plugin probably specifically asked to not be
						// added
					}
				}
			}
		...
		if (configMan.getBooleanParameter(sPropertiesPrefix
				+ ".subViews.minimized", false)) {
			tabFolder.setMinimized(true);
			tabFolderData.height = iFolderHeightAdj;
		} else {
	...
	/**
	 * Creates a composite within the specified composite and sets its layout to
	 * a default FillLayout().
	 * 
	 * @param composite
	...
	public void initializeTable(final Table table) {
		...
		table.addMouseListener(new MouseAdapter() {
			private TableCellMouseEvent createMouseEvent(TableCellCore cell,
				...
				event.x = e.x - r.x
						+ edu.northwestern.ono.tables.utils.VerticalAligner.getTableAdjustHorizontallyBy(table);
				event.y = e.y - r.y
						+ edu.northwestern.ono.tables.utils.VerticalAligner.getTableAdjustVerticalBy(table);
				return event;
			}
			...
			public void mouseDoubleClick(MouseEvent e) {
				...
				if (cell != null && tc != null) {
					...
					if (event.skipCoreFunctionality)
						lCancelSelectionTriggeredOn = System
								.currentTimeMillis();
				}
			}
			...
			public void mouseDown(MouseEvent e) {
				...
				try {
					...
					if (rTableArea.contains(pMousePosition)) {
						...
						Rectangle cellBounds = ti
								.getBounds(columnOrder[columnOrder.length - 1]);
						// OSX returns 0 size if the cell is not on screen
						...
						/*
						 * // This doesn't work because of OS inconsistencies
						 * when table is scrolled // Re-enable once SWT fixes
						 * the problem // Bug 103934: Table.getItem(Point) uses
						 * incorrect calculation on Motif // Fixed 20050718 SWT
						 * 3.2M1 (3201) & SWT 3.1.1 (3139) // TODO: Get Build
						 * IDs and use this code (if it works) TableItem ti =
						 * table.getItem(pMousePosition); if (ti == null)
						 * table.deselectAll();
						 */
					}
				} catch (Exception ex) {
			...
			public void mouseMove(MouseEvent e) {
				...
				try {
					...
					if (iCursorID != lastCursorID) {
						...
						if (iCursorID >= 0) {
							table.setCursor(table.getDisplay().getSystemCursor(
									iCursorID));
						} else {
		...
		table.addSelectionListener(new SelectionListener() {
			public void widgetSelected(SelectionEvent event) {
				...
				// Set Data Object for all tabs. Tabs of PluginView are sent the
				// plugin
				// Peer object, while Tabs of IView are sent the core PEPeer
				// object.
				...
				for (int i = 0; i < tabViews.size(); i++) {
					...
					if (view != null) {
						if (view instanceof UISWTViewImpl) {
							...
							view
									.dataSourceChanged(dataSourcesCore.length == 0 ? null
											: dataSourcesCore);
						}
					}
				}
			...
			public void widgetDefaultSelected(SelectionEvent e) {
				if (lCancelSelectionTriggeredOn > 0
						&& System.currentTimeMillis()
								- lCancelSelectionTriggeredOn < 200) {
					e.doit = false;
		...
		if (bTableVirtual || false)
			table.addListener(SWT.SetData, new Listener() {
				public void handleEvent(Event e) {
					...
					// This is catch is temporary for SWT 3212, because there
					// are cases where
					...
					if (row == null || row.getIndex() != tableIndex) {
						// System.out.println("SetData " + tableIndex + ":
						// Sort..");
						fillRowGaps(false);

						row = (TableRowCore) item.getData("TableRow");
						if (row == null || row.getIndex() != tableIndex) {
							// row's been deleted. tableitem probably about to
							// be remove
							// (hopefully!)
							if (DEBUGADDREMOVE)
								Debug.outStackTrace();
					...
					if (!Utils.TABLE_GRIDLINE_IS_ALTERNATING_COLOR) {
						...
						table.redraw(0, r.y, table.getClientArea().width,
								r.height, false);
					}
				}
			});
		...
		/*
		 * Temporarily Disabled to see if we need it anymore if(Constants.isOSX) {
		 * table.addSelectionListener(new SelectionAdapter() { public void
		 * widgetSelected( SelectionEvent event) { GroupTableRowRunner refresher =
		 * new GroupTableRowRunner() { public void run(TableRowCore row) {
		 * row.setValid(false); row.refresh(true); } };
		 * 
		 * TableItem[] sel = table.getSelection();
		 * 
		 * ArrayList toRefresh = new ArrayList(sel.length);
		 * 
		 * if(oldSelectedItems != null) { runForTableItems(oldSelectedItems,
		 * refresher); for (int i = 0; i < sel.length; i++) {
		 * if(!oldSelectedItems.contains(sel[i])) toRefresh.add(sel[i]); } }
		 * else { for (int i = 0; i < sel.length; i++) { toRefresh.add(sel[i]); } }
		 * 
	...
	protected void paintItem(Event event) {
		...
		try {
			...
			TableCellCore cell = row.getTableCellCore(columnsOrdered[iColumnNo]
					.getName());
			...
			// System.out.println("PS " + table.indexOf(item) + ";" +
			// cellBounds);
			GCStringPrinter.printString(event.gc, cell.getText(), cellBounds,
					true, true, columnsOrdered[iColumnNo].getSWTAlign());
	...
	protected void initializeTableColumns(final Table table) {
		...
		ControlListener resizeListener = new ControlAdapter() {
			...
			public void controlResized(ControlEvent e) {
				...
				try {
					...
					TableColumnCore tc = (TableColumnCore) column
		...
		// this will be removed when a SWT-provided solution is available to
		// satisfy all platforms with identation issue
		...
		for (int i = 0; i < tableColumns.length; i++) {
			...
			// user has reported a problem here with index-out-of-bounds - not
			// sure why
			// but putting in a preventative check so that hopefully the view
			// still opens
			// so they can fix it
			...
			if (adjusted_position >= numSWTColumns) {
				Debug.out("Incorrect table column setup, skipping column '"
						+ sName + "', position=" + adjusted_position
						+ ";numCols=" + numSWTColumns);
				continue;
			}
			...
			Messages.setLanguageText(column, tableColumns[i]
					.getTitleLanguageKey());
		...
		TableColumnManager tcManager = TableColumnManager.getInstance();
		TableColumnCore tc = tcManager
				.getTableColumnCore(sTableID, sSortColumn);
		if (tc == null) {
			tc = tableColumns[0];
		}
		sortColumn = tc;
		...
		// Add move listener at the very end, so we don't get a bazillion
		// useless
		// move triggers
		if (SWT.getVersion() >= 3100) {
	...
	private void addTableContextMenuItems(
			org.gudy.azureus2.plugins.ui.menus.MenuItem[] items, Menu parent,
			boolean prev_was_separator, boolean enable_items) {
		for (int i = 0; i < items.length; i++) {
			...
			org.gudy.azureus2.plugins.ui.menus.MenuItem[] child_items = contextMenuItem
					.getItems();
						...
						public boolean run(TableRowCore[] rows) {
							if (swt_style == SWT.CHECK
								...
								if (!menuItem.isDisposed()) {
									contextMenuItem.setData(new Boolean(
			...
			if (is_container) {
				Menu this_menu = new Menu(getComposite().getShell(),
						SWT.DROP_DOWN);
				menuItem.setMenu(this_menu);
				this.addTableContextMenuItems(child_items, this_menu, false,
						enable_items);
			}

			if (enable_items) {
				contextMenuItem
						.invokeMenuWillBeShownListeners(getSelectedRows());
	...
	private void addThisColumnSubMenu(int iColumn) {
		...
		item.addListener(SWT.Selection, new Listener() {
			public void handleEvent(Event e) {
				...
				table.getColumn(iColumn).notifyListeners(SWT.Selection,
						new Event());
			}
		});
		...
		Messages.setLanguageText(item,
				"MyTorrentsView.menu.thisColumn.toClipboard");
		item.addListener(SWT.Selection, new Listener() {
			public void handleEvent(Event e) {
				String sToClipboard = "";
				int iColumn = ((Long) menu.getData("ColumnNo")).intValue();
		...
		// Add Plugin Context menus..
		TableColumnCore tc = (TableColumnCore) tcColumn
				.getData("TableColumnCore");
		...
		if (items.length > 0) {
			...
			for (int i = 0; i < items.length; i++) {
				...
				Messages.setLanguageText(menuItem, contextMenuItem
						.getResourceKey());
				menuItem.addListener(SWT.Selection,
						new SelectedTableRowsListener() {
							public boolean run(TableRowCore[] rows) {
	...
	/**
	 * Create a SubMenu for column specific tasks. Everytime the user opens the
	 * context menu, the "This Column" submenu is cleared, and this function is
	 * called to refill it.
	 * 
	 * @param sColumnName
	 *            The name of the column the user clicked on
	 * @param menuThisColumn
	 *            the menu to fill with MenuItems
	 */
	public void addThisColumnSubMenu(String sColumnName, Menu menuThisColumn) {
		/*
		 * // Template code if (sColumnName.equals("xxx")) { item = new
		 * MenuItem(menuThisColumn, SWT.PUSH); Messages.setLanguageText(item,
		 * "xxx.menu.xxx"); item.setImage(ImageRepository.getImage("xxx"));
		 * item.addListener(SWT.Selection, new Listener() { public void
		 * handleEvent(Event e) { // Code here } });
		 */
	}
		...
		try {
			...
			if (checkColumnWidthsEvery != 0
				...
				for (int i = 0; i < tableColumnsSWT.length; i++) {
					TableColumnCore tc = (TableColumnCore) tableColumnsSWT[i]
							.getData("TableColumnCore");
					if (tc != null
							&& tc.getWidth() != tableColumnsSWT[i].getWidth()) {
			...
			if (bWillSort) {
				if (bForceSort) {
					sortColumn.setLastSortValueChange(SystemTime
							.getCurrentTime());
				}
				sortColumn(true);
			}
		...
		try {
			...
			if (dataSourcesToRemove.size() > 0) {
				...
				if (DEBUGADDREMOVE && dataSourcesRemove.length > 1)
					debug("Streamlining removing " + dataSourcesRemove.length
							+ " rows");
	...
	/**
	 * IView.delete: This method is called when the view is destroyed. Each
	 * color instanciated, images and such things should be disposed. The caller
	 * is the GUI thread.
	 */
	public void delete() {
		if (tabViews != null && tabViews.size() > 0) {
	 ...
	 * @param bImmediate
	 *            Add immediately, or queue and add at next refresh
	 */
		...
		try {
			...
			if (DEBUGADDREMOVE)
				debug("Queued " + count + " of " + dataSources.length
						+ " dataSources to add.  Total Queued: "
	...
	private void refreshenProcessDataSourcesTimer() {
		...
		synchronized (timerProcessDataSources) {
			if (timerEventProcessDS != null && !timerEventProcessDS.hasRun()) {
				// Push timer forward, unless we've pushed it forward for over x
				// seconds
				...
				timerEventProcessDS = timerProcessDataSources
						.addEvent(SystemTime.getCurrentTime()
								+ IMMEDIATE_ADDREMOVE_DELAY,
	...
	private void reallyAddDataSources(final Object dataSources[]) {
		...
		if (mainComposite == null || table == null
		...
		try {
			...
			for (int i = 0; i < dataSources.length; i++) {
				...
				// Break off and add the rows to the UI if we've taken too long
				// to
				// create them
				if (SystemTime.getCurrentTime() - lStartTime > BREAKOFF_ADDTOMAP) {
					...
					if (DEBUGADDREMOVE) {
						debug("Breaking off adding datasources to map after "
								+ i + " took "
								+ (SystemTime.getCurrentTime() - lStartTime)
								+ "ms; # remaining: " + iNewSize);
					}
					remainingDataSources = new Object[iNewSize];
					doneDataSources = new Object[i];
					System.arraycopy(dataSources, i, remainingDataSources, 0,
							iNewSize);
				...
				if (dataSourceToRow.containsKey(dataSources[i])) {
					...
					TableRowImpl row = new TableRowImpl(table, sTableID,
		...
		if (remainingDataSources == null) {
			...
			// remaining datasources will be on SWT thread. OSX has horrible
			// handling
	...
	private void _addDataSourcesToSWT(final Object dataSources[]) {
		...
		try {
			...
			int iBottomIndex = Utils.getTableBottomIndex((TableOrTreeSWT) table, iTopIndex);
			...
			for (int i = 0; i < dataSources.length; i++) {
				...
				TableRowImpl row = (TableRowImpl) dataSourceToRow
						.get(dataSource);
				if (row == null || row.getIndex() >= 0)
					continue;
				if (sortColumn != null) {
					TableCellCore cell = row.getTableCellCore(sortColumn
							.getName());
					if (cell != null) {
						try {
							cell.invalidate();
							...
							Logger.log(new LogEvent(LOGID,
									"Minor error adding a row to table "
											+ sTableID, e));
						}
					}
				}
				...
				try {
					...
					if (sortedRows.size() > 0) {
						// If we are >= to the last item, then just add it to
						// the end
						// instead of relying on binarySearch, which may return
						// an item
						// in the middle that also is equal.
						TableRowCore lastRow = (TableRowCore) sortedRows
								.get(sortedRows.size() - 1);
						if (sortColumn.compare(row, lastRow) >= 0) {
							...
							if (DEBUGADDREMOVE)
								debug("Adding new row at position " + index
					...
					if (!bReplacedVisible && index >= iTopIndex
							&& index <= iBottomIndex) {
						bReplacedVisible = true;
					}

					// XXX Don't set table item here, it will mess up selected
					// rows
					// handling (which is handled in fillRowGaps called later
					// on)
					// row.setTableItem(index);
					row.setIconSize(ptIconSize);
				} catch (Exception e) {
					Logger.log(new LogEvent(LOGID,
							"Error adding a row to table " + sTableID, e));
					try {
		...
		boolean ok = Utils.execSWTThread(new AERunnable() {
			public void runSupport() {
				...
				for (int i = 0; i < dataSources.length; i++) {
					...
					TableRowCore item = (TableRowCore) dataSourceToRow
							.remove(dataSources[i]);
					if (item != null) {
						if (!bRefresh) {
							int index = item.getIndex();
							bRefresh = index >= iTopIndex
									|| index <= iBottomIndex;
						}
				...
				// Remove the rows from SWT first. On SWT 3.2, this currently
				// has
				// zero perf gain, and a small perf gain on Windows. However, in
				// the
	...
	public void columnOrderChanged(int[] positions) {
		try {
			...
			// Pre SWT 3.1
			// This shouldn't really happen, since this function only gets
			// triggered
			// from SWT >= 3.1
	...
	public void columnInvalidate(final String sColumnName) {
		TableColumnCore tc = TableColumnManager.getInstance()
				.getTableColumnCore(sTableID, sColumnName);
		if (tc != null)
	 ...
	 * </pre>
	 * 
	 * s, in the last sorted order. The order will not be of the supplied cell's
	 * sort unless the table has been sorted by that column previously.
	 * <p>
	 * ie. You can sort on the 5th column, and retrieve the cells for the 3rd
	 * column, but they will be in order of the 5th columns sort.
	 * 
	 * @param sColumnName
	 *            Which column cell's to return. This does not sort the array on
	 *            the column.
	 ...
	 * 
	 * @param dataSource
	 *            a reference to a core Datasource object (not a plugin
	 *            datasource object)
	 ...
	 *       computePossibleActions isn't being calculated right because of
	 *       non-created rows when select user selects all
	 */
	 ...
	 * 
	 * @param a
	 *            the array into which the selected data sources are to be
	 *            stored, if the size is the big enough; otherwise, a new array
	...
	public TableRowCore[] getSelectedRows() {
		return (TableRowCore[]) getSelectedRowsList().toArray(
				new TableRowCore[0]);
	}

	/**
	 * Returns an list of all selected TableRowCore objects. Null data sources
	 * are ommitted.
	 * 
	...
	public Object getFirstSelectedDataSource(boolean bCoreObject) {
		...
		TableRowCore row = (TableRowCore) table.getSelection()[0]
				.getData("TableRow");
		if (row == null)
			return null;
	...
	/**
	 * For each row source that the user has selected, run the code provided by
	...
	public void runForSelectedRows(GroupTableRowRunner runner) {
		...
		if (rows_to_use != null) {
			runner.run((TableRowCore[]) rows_to_use
					.toArray(new TableRowCore[rows_to_use.size()]));
		}
	}
	...
	public void runForAllRows(GroupTableRowRunner runner) {
		// put to array instead of synchronised iterator, so that runner can
		// remove
	...
	public void runForAllRows(GroupTableRowVisibilityRunner runner) {
		...
		for (int i = 0; i < rows.length; i++) {
			...
			runner.run(rows[i], true /*index >= iTopIndex && index <= iBottomIndex*/);
		}
	}
	 ...
	 * 
	 * @param items
	 *            A list of TableItems that are part of the table view
	 * @param runner
	 *            A task
	 */
	...
	public void clipboardSelected() {
		...
		new Clipboard(getComposite().getDisplay()).setContents(
				new Object[] { sToClipboard }, new Transfer[] { TextTransfer
						.getInstance() });
	}
	...
	public static abstract class GroupTableRowRunner {
		...
		private boolean supportsMultipleRows() {
			return run(EMPTY);
		}
	 ...
	 */
	public abstract class SelectedTableRowsListener extends GroupTableRowRunner
			implements Listener {
		/**
		...
		/**
		 * Process the trapped event. This function does not need to be
		 * overidden.
		 * 
		 * @param e
		 *            event information
	...
	private class ColumnSelectionListener implements Listener {
		...
		public void handleEvent(final Event event) {
			if (COLUMN_CLICK_DELAY) {
				// temporary for OSX.. resizing column triggers selection, so
				// cancel
	...
	/**
	 * Handle movement of a column based on user dragging the Column Header. SWT >=
	 * 3.1
	 */
	private class ColumnMoveListener implements Listener {
		public void handleEvent(Event event) {
			...
			// It would have been easier if event (.start, .end) contained the
			// old
			// and new position..
		...
		public void handleEvent(Event event) {
			...
			case SWT.MouseHover: {
				...
				if (cell == null)
					return;
				cell
						.invokeToolTipListeners(TableCellCore.TOOLTIPLISTENER_HOVER);
				...
				toolTipShell.setBackground(d
						.getSystemColor(SWT.COLOR_INFO_BACKGROUND));

				toolTipLabel = new Label(toolTipShell, SWT.WRAP);
				toolTipLabel.setForeground(d
						.getSystemColor(SWT.COLOR_INFO_FOREGROUND));
				toolTipLabel.setBackground(d
						.getSystemColor(SWT.COLOR_INFO_BACKGROUND));
	...
	private int getColumnNo(int iMouseX) {
		...
		if (table.getItemCount() > 0) {
			...
			for (int i = bSkipFirstColumn ? 1 : 0; i < table.getColumnCount(); i++) {
				...
				// "TableItem.getBounds(int) returns the wrong values when table
				// scrolled"
	...
	private void _sortColumn(boolean bForceDataRefresh, boolean bFillGapsOnly,
		...
		try {
			...
			if (iFocusIndex == table.getItemCount())
				iFocusIndex--;
			TableRowCore focusedRow = (iFocusIndex == -1 || table.getItemCount()==0) ? null
					: (TableRowCore) table.getItem(iFocusIndex).getData(
							"TableRow");
			...
			for (int i = 0; i < selectedRowIndices.length; i++) {
				selectedRows[i] = (TableRowCore) table.getItem(
						selectedRowIndices[i]).getData("TableRow");
			}
				...
				if (!bFillGapsOnly) {
					if (sortColumn != null
						...
						if (DEBUG_SORTER) {
							long lTimeDiff = (System.currentTimeMillis() - lTimeStart);
							if (lTimeDiff > 150)
								System.out.println("--- Build & Sort took "
										+ lTimeDiff + "ms");
						}
					} else {
						if (DEBUG_SORTER) {
			...
			/**
			 * SWT/Windows Bug: When we set selection, the first index is the
			 * focus row. This works visually, however, if you press shift-up or
			 * shift-down, it uses an older selection index.
			 * 
			 * ie. User selects row #10 Programmically change selection to Row
			 * #15 only Shift-down Rows 10 through 26 will be selected
			 * 
			...
			if (focusedRow != null) {
				...
				if (numSame < selectedRows.length) {
					// XXX setSelection calls showSelection(). We don't want the
					// table
			...
			if (DEBUG_SORTER) {
				long lTimeDiff = (System.currentTimeMillis() - lTimeStart);
				if (lTimeDiff >= 500)
					System.out.println("<<< Sort & Assign took " + lTimeDiff
							+ "ms with " + iNumMoves + " rows (of "
							+ sortedRows.size() + ") moved. " + focusedRow
							+ ";" + Debug.getCompressedStackTrace());
			}
		} finally {
			sortColumn_mon.exit();
	...
	public void sortColumnReverse(TableColumnCore sorter) {
		...
		if (!bSameColumn) {
			...
			configMan.setParameter(sTableID + ".sortAsc", sortColumn
					.isSortAscending());
			configMan.setParameter(sTableID + ".sortColumn", sortColumn
					.getName());
		} else {
			sortColumn.setSortAscending(!sortColumn.isSortAscending());
			configMan.setParameter(sTableID + ".sortAsc", sortColumn
	...
	private void changeColumnIndicator() {
		...
		try {
			// can't use TableColumnCore.getPosition, because user may have
			// moved
			// columns around, messing up the SWT column indexes.
			// We can either use search columnsOrdered, or search
			// table.getColumns()
			...
			for (int i = 0; i < tcs.length; i++) {
				...
				if (sName != null && sName.equals(sortColumn.getName())) {
					table
							.setSortDirection(sortColumn.isSortAscending() ? SWT.UP
									: SWT.DOWN);
	...
	private void visibleRowsChanged() {
		...
		if (lastTopIndex != iTopIndex) {
			...
			if (iTopIndex < tmpIndex) {
				...
				// System.out.println("Refresh top rows " + iTopIndex + " to " +
				// (tmpIndex - 1));
				try {
					sortedRows_mon.enter();
					for (int i = iTopIndex; i < tmpIndex
							&& i < sortedRows.size(); i++) {
						TableRowCore row = (TableRowCore) sortedRows.get(i);
						row.refresh(true, true);
						row.setAlternatingBGColor(true);
				...
				iBottomIndex = Utils.getTableBottomIndex((TableOrTreeSWT) table, iTopIndex);
			} else {
				// System.out.println("Made T.Invisible " + (tmpIndex) + " to "
				// + (iTopIndex - 1));
			}
		}
		...
		if (lastBottomIndex != iBottomIndex) {
			...
			if (tmpIndex <= iBottomIndex) {
				// System.out.println("Refresh bottom rows " + (tmpIndex + 1) +
				// " to " + iBottomIndex);
				try {
					sortedRows_mon.enter();
				...
				// System.out.println("Made B.Invisible " + (tmpIndex) + " to "
				// + (iBottomIndex + 1));
			}
		}
	...
	public Image obfusticatedImage(final Image image, Point shellOffset) {
		...
		TableColumn[] tableColumnsSWT = table.getColumns();
		for (int i = 0; i < tableColumnsSWT.length; i++) {
			final TableColumnCore tc = (TableColumnCore) tableColumnsSWT[i]
					.getData("TableColumnCore");
			...
			if (tc != null && tc.isObfusticated()) {
				...
				for (int j = iTopIndex; j <= iBottomIndex; j++) {
					...
					TableRowCore row = (TableRowCore) table.getItem(j).getData(
							"TableRow");
					if (row != null) {
						...
						final Point offset = table.toDisplay(columnBounds.x,
								columnBounds.y);
						...
						if (text != null) {
							UIDebugGenerator.obfusticateArea(
									table.getDisplay(), image, columnBounds,
									text);
						}
					}
				}
	...
	private void debug(String s) {
		System.out.println(SystemTime.getCurrentTime() + ": " + sTableID + ": "
				+ s);
	}

	public Image obfusticatedImage(Image arg0) {
