 ...
 * RCS:          $Id: EdgeMapTable.java,v 1.2 2010/03/29 16:48:04 drc915 Exp $
...
import edu.northwestern.ono.vuze.ui.items.CustomerItem;
import edu.northwestern.ono.vuze.ui.items.IpItem;
import edu.northwestern.ono.vuze.ui.items.PctOfTimeSeenItem;
import edu.northwestern.ono.vuze.ui.items.RttItem;
...
public class EdgeMapTable 
       extends TableViewSWTImpl<EdgeData>
{
...
private EdgeData[] edgesToRemove;
private EdgeData[] edgesToAdd;
  ...
  public EdgeMapTable(OnoView onoView) {	  
	    super(EdgeData.class, OnoView.EDGE_MAPPINGS, "EdgeMappings", 
	          basicItems, "cust");
    setRowDefaultHeight(16);
    this.onoView = onoView;
    bEnableTabViews = false;
    edges = new ArrayList<EdgeData>();
    edgesToRemove = new EdgeData[0];
    edgesToAdd = new EdgeData[0];
    ...
    //1. Unregister for item creation
		super.tableStructureChanged(bEnableTabViews, onoView.getClass());
  }
...
//		new MenuItem(menu, SWT.SEPARATOR);
  ...
  public void delete() {
	...
	private void updateExistingData(EdgeData[] dataSources) {
		...
		edgesToAdd = new EdgeData[toAdd.size()];
		for (int i = 0; i < toAdd.size(); i++) edgesToAdd[i] = toAdd.get(i);
		edgesToRemove = new EdgeData[toRemove.size()];
